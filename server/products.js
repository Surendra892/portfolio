...
app.get("/category/:categoryname",(req,res)=>{
    ...
    mongoclient.connect(connectionstring,(err,clientObj)=>{
        if(!err){
            var database= clientObj.db("demodb");
            database.collection("tblProducts").find({Category:CategoryName}).toArray((err,documents)=>{
                if(!err) {
                    res.send(documents);
                    res.end();
                }
...
app.patch("/updatedata/:id",(req,res)=>{
    ...
    mongoclient.connect(connectionstring,(err,clientObj)=>{
        if(!err){
            var database = clientObj.db("demodb");
            database.collection("tblProducts").updateOne({Productid:id},{$set:{Name:(req.body.Name),Price:parseFloat(req.body.Price),Stock:(req.body.Stock=="true")?true:false,Category:(req.body.Category),Rating:parseFloat(req.body.Rating)}},(err,result)=>{
                if(!err){
